...
Админ-панель для VPN-бота: аутентификация по JWT, редактируемые шаблоны сообщений (с произвольными типами/флагами) и статистика по событиям и их конверсии.
...
> Важно: `POST /api/ingest/messages` (лог каждой отдельной отправки) убран — статистика теперь считается на уровне шаблона/события, а не построчным логом каждого сообщения каждому юзеру. Если в `shredder-action-controll` остался вызов `admin_client.log_message(...)`, он будет получать 404 — это не критично (вызов обёрнут в try/except, просто шум в логах), но вызов стоит убрать при следующей правке того сервиса.
...
- `GET/POST/PATCH/DELETE /api/templates` — CRUD шаблонов сообщений (все сообщения бота, включая дефолтные, с редактируемым текстом, типом и опциональной привязкой к типу события для статистики).
- `GET/POST /api/template-types` — список типов/флагов шаблона (`regular`, `alert`, ...) и создание новых прямо из админки, без деплоя.
